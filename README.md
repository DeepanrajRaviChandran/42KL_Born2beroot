...
### Set up a service of your choice (SFTP)
- FTP stands for File Transfer Protocol. It is a standard network protocol used to transfer files between a client and a server on a computer network, typically the internet or a local area network (LAN). FTP is one of the oldest and most widely used methods for file transfer, dating back to the early days of the internet.
- The FTP protocol operates on a client-server model, where one computer acts as the client and initiates the file transfer, while another computer acts as the server and provides access to files and directories. The client can upload (send) files to the server or download (receive) files from the server.
- Key features and characteristics of FTP include:
    1. Authentication: FTP requires user authentication (username and password) to access files on the server. Some FTP servers allow anonymous access, where users can connect without providing login credentials.
    2. Two Modes: FTP operates in two modes: Active mode and Passive mode. These modes determine how data connections are established between the client and the server.
    3. Commands: FTP uses various commands to perform actions such as navigating directories, uploading files, downloading files, deleting files, and changing file permissions.
    4. Port Number: FTP typically operates on port 21. However, when data transfer is required, a separate port (usually port 20 for active mode and a randomly chosen port for passive mode) is used for data communication.
    5. Security Considerations: Traditional FTP sends data, including login credentials, in plain text, which can be a security risk. To address this concern, secure alternatives like FTPS (FTP Secure) and SFTP (SSH File Transfer Protocol) were introduced, providing encryption for data transfer.
    6. Graphical FTP Clients: While FTP can be used from the command line, many users prefer graphical FTP clients that provide a user-friendly interface for connecting to FTP servers, managing files, and transferring data.
- FTP is commonly used for various purposes, such as:
    - Uploading website files to a web server.
    - Downloading files from a remote server to a local machine.
    - Sharing large files with others over the internet.
    - Backing up files to a remote server.
    - Transferring files between computers within a local network.
- However, due to its lack of built-in security and potential for data interception, alternatives like FTPS and SFTP are often preferred for secure file transfer needs.
